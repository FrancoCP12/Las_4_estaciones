...
public class Modificar extends javax.swing.JFrame {
    ...
    public Modificar(com.Las_4_Estaciones.management.persistencia.Datos.Reserva reserva) {
        ...
        DniPasaporte.setEditable(false);
        Nombre.setText(reserva.getCliente().getNombre());
        Apellido.setText(reserva.getCliente().getApellido());
        Telefono.setText(String.valueOf(reserva.getCliente().getTelefono()));
        DniPasaporte.setText(String.valueOf(reserva.getCliente().getDniPasaporte()));
        System.out.println(reserva.getMetodoPago());
        MetodosPago.setSelectedItem(String.valueOf(reserva.getMetodoPago()));
        Departamentos.setSelectedItem(String.valueOf(reserva.getDepartamento()));
        Direccion.setText(reserva.getCliente().getDireccion());
        Email.setText(reserva.getCliente().getEmail());
        CtdAdultos.setSelectedItem(String.valueOf(reserva.getCtdAdultos()));
        CtdMenores.setSelectedItem(String.valueOf(reserva.getCtdMenores()));
        FechaIngreso.setDate(reserva.getCheckIn());
        FechaEgreso.setDate(reserva.getCheckOut());
        HoraLlegada.setText(reserva.getVehiculo().getHoraLlegada());
        HoraPartida.setText(reserva.getVehiculo().getHoraPartida());
        Origen.setText(reserva.getVehiculo().getOrigen());
        TipoVehiculo transporte = reserva.getVehiculo().getTipoVehiculo();
        
        if (transporte.equals(COLECTIVO) || transporte.equals(AVION)){
            VehiculoContratado vehiculo = (VehiculoContratado) reserva.getVehiculo();
            Empresa.setText(vehiculo.getEmpresa());
            if(transporte.equals(COLECTIVO)){
                Colectivo.setSelected(true);
            }
        ...
        }else if(transporte.equals(MOTOCICLETA) || transporte.equals(CAMIONETA) || transporte.equals(AUTOMOVIL)){
            VehiculoPropio vehiculo = (VehiculoPropio) reserva.getVehiculo();
            Modelo.setText(vehiculo.getMarcaModelo());
            Patente.setText(vehiculo.getPatente());
            if(transporte.equals(MOTOCICLETA)){
                Motocicleta.setSelected(true);
            }
            else if(transporte.equals(CAMIONETA)){
                Camioneta.setSelected(true);
            }
            else if(transporte.equals(AUTOMOVIL)){
                Automovil.setSelected(true);
            }
    ...
    private void GuardarMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_GuardarMouseClicked
        ...
        if(!verificarNombre && !verificarApellido && !verificarDireccion && !verificarDniPasaporte &&
           ...
           ((!verificarEmpresa)||(!verificarPatente && !verificarModelo))){
            ...
            Cliente cliente;
            cliente = new Cliente(Nombre.getText(),Apellido.getText(), DniPasaporte.getText(), Long.parseLong(Telefono.getText()),Direccion.getText(), Email.getText());
            ...
            if(!verificarEmpresa){
                ...
                nuevaReserva = new Reserva(cliente,FechaIngreso.getDate(), FechaEgreso.getDate(),Departamento.valueOf((String) Departamentos.getSelectedItem()), vehiculo, MetodoPago.valueOf((String) MetodosPago.getSelectedItem()), Integer.parseInt((String) CtdAdultos.getSelectedItem()), Integer.parseInt((String) CtdMenores.getSelectedItem()));
            }else{
            ...
            try {
                if(validacion.validacionReserva(this.reserva, nuevaReserva)){
                    Consultas.updateReserva(nuevaReserva, this.reserva);
    ...
    private void modificarBarraMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_modificarBarraMouseClicked
        this.dispose();
        BuscarModificar BuscarPantalla = new BuscarModificar();
        BuscarPantalla.setVisible(true);
        BuscarPantalla.setLocationRelativeTo(null);
